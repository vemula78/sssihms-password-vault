"""EFF short wordlist 2.0, vendored verbatim (DESIGN.md §5).

1,296 words, 4 dice rolls per word (~10.34 bits/word). Source:
https://www.eff.org/files/2016/09/08/eff_short_wordlist_2_0.txt — the dice-roll code
column is dropped, the word column kept, in file order. Vendored rather than fetched at
runtime: the bench VM is proxy-restricted, and a runtime fetch would be both a dependency
and a tamper surface for something that feeds directly into generated secrets.

A unit test (``vault/tests/test_generator.py``) asserts ``len(WORDLIST) == 1296`` and that
every entry is lowercase ASCII with no duplicates — do not hand-edit this file without
